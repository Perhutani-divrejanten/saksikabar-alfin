# Final Verification Script - Memastikan rebrand Janten Bersuara selesai dengan baik

$WorkspaceRoot = "c:\KULIAH\MAGANG\Magang di Perhutani\Janten Bersuara"

Write-Host "========== FINAL VERIFICATION - Janten Bersuara REBRAND ==========" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "4. Checking for new branding 'Janten Bersuara'..." -ForegroundColor Yellow
$newBrandingFound = Get-ChildItem -Path $WorkspaceRoot -Recurse -Include "*.html" -File |
    Select-String -Pattern "Janten Bersuara|jantenbersuara|jantenbersuara" -ErrorAction SilentlyContinue |
    Where-Object { $_.Path -notlike "*\.bak.*" } | 
    Measure-Object

if ($newBrandingFound.Count -gt 0) {
    Write-Host "   ✅ Found 'Janten Bersuara' branding in $($newBrandingFound.Count) places" -ForegroundColor Green
} else {
    Write-Host "   ⚠️  No 'Janten Bersuara' branding found!" -ForegroundColor Yellow
    $issues += @{ Type = "Missing"; File = "All"; Reason = "No Janten Bersuara branding found" }
}
...
foreach ($pkg in $pkgFiles) {
    ...
    if ($content -match '"name"\s*:\s*"jantenbersuara') {
        $pkgOK++
...
Write-Host ""
Write-Host "Rebrand Janten Bersuara SELESAI" -ForegroundColor Green
